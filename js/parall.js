...
$(document).ready(function() { //when the document is ready...
	...
	var $technikBG = $('#oreginal');
	var techniksprite = $('#technik .sprite');
	var $labBG = $('#lab');
	var labsprite = $('#lab .sprite');
	var $aboutBG = $('#aboutUs');
	var aboutsprite = $('#aboutUs .sprite');
	var $jobsBG = $('#jobs');

	// yy+
	var $shopLifeBG=$('#shopLife');
	var shopLife=$('#shopLife .sprite');
	
	var windowHeight = $window.height(); //get the height of the window
	
	//apply the class "inview" to a section that is in the viewport
	$("#home,#projects,#oreginal,#shopLife,#aboutUs,#jobs").bind("inview", function (event, visible) {
	...
	*/
	function newPos(x, windowHeight, pos, adjuster, inertia){
		return x + "% " + (((windowHeight + pos) - adjuster) * inertia)/5  + "px";
	}
	...
	function Move(){ 
		...
		if($homeBG.hasClass("inview")){
			...
			//var thisT=shopLife.css("backgroundPosition-y");
			homesprite.css({'backgroundPosition': newPos(50, windowHeight, pos, 0, 0.6)});
			//call the newPos function and change the secnond background position
			
		}
		if($shopLifeBG.hasClass("inview")){
			// $shopLifeBG.css({'backgroundPosition': newPos(50, windowHeight, pos, 600, 0.3)});
			//var thisT=shopLife.css("backgroundPosition-y");
			shopLife.css({'backgroundPosition': newPos(80, windowHeight, pos, 0, 0.6)});
			//call the newPos function and change the secnond background position
		...
		if( $aboutBG.hasClass("inview")){
			...
			//var thisT=shopLife.css("backgroundPosition-y");
			aboutsprite.css({'backgroundPosition': newPos(50, windowHeight, pos, 0, 0.6)});
			//call the newPos function and change the secnond background position
		...
		/*if($homeBG.hasClass("inview")){
			...
			// $homeBG.css({'backgroundPosition': newPos(50, windowHeight, pos, 1000, 0.3)}); 
			//call the newPos function and change the secnond background position
			homesprite.css({'backgroundPosition': newPos(50, windowHeight, pos, 911, 0.6)});
		}
*/		
		// projects: 1000
		/*if($projectsBG.hasClass("inview")){
			//call the newPos function and change the background position
			$projectsBG.css({'backgroundPosition': newPos(50, windowHeight, pos, 1568, 0.3)});
		}
		*/
		// design: 2000
...
*/
		...
		// 	//call the newPos function and change the secnond background position
		// 	aboutsprite.css({'backgroundPosition': newPos(50, windowHeight, pos, 5350, 0.6)});
		// }

		// // jobs: 6000
		// if($shopLifeBG.hasClass("inview")){
		// 	//call the newPos function and change the background position
		// 	$shopLife.css({'backgroundPosition': newPos(50, windowHeight, pos, 4000, 0.3)}); 
		// }
